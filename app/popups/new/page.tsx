...
import { Save, X, Type, Image as ImageIcon, FileText, Eye, Plus, Trash2, ChevronDown, ChevronUp } from "lucide-react"
...
export default function NewPopupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "הצלחה",
          description: "הפופאפ נוצר בהצלחה",
        })
        router.push("/popups")
      } else {
        ...
        toast({
          title: "שגיאה",
          description: error.error || "אירעה שגיאה ביצירת הפופאפ",
          variant: "destructive",
        })
      }
    } catch (error) {
      ...
      toast({
        title: "שגיאה",
        description: "אירעה שגיאה ביצירת הפופאפ",
        variant: "destructive",
      })
    } finally {
      setSaving(false)
  ...
  return (
    <AppLayout title="יצירת פופאפ">
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
          {/* Header */}
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">יצירת פופאפ</h1>
              <p className="text-gray-600 mt-1">
                הגדר את הפופאפ שלך כדי לעודד פעולות מהמבקרים
              </p>
            </div>
