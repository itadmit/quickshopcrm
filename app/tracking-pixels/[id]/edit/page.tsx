...
export default function EditTrackingPixelPage() {
  const router = useRouter()
  const params = useParams()
  const { toast } = useToast()
  const pixelId = params.id as string
  ...
  const fetchPixel = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "שגיאה",
          description: "לא ניתן לטעון פיקסל",
          variant: "destructive",
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "הצלחה",
          description: "פיקסל עודכן בהצלחה",
        })
        router.push("/tracking-pixels")
      } else {
        ...
        toast({
          title: "שגיאה",
          description: error.error || "לא ניתן לעדכן פיקסל",
          variant: "destructive",
        })
      }
    } catch (error) {
    ...
    <AppLayout>
      <div className="p-6">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">עריכת פיקסל</h1>
          <p className="text-gray-600 mt-2">
            עדכן את הגדרות הפיקסל
          </p>
        </div>
            ...
            <CardContent className="space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  לא ניתן לשנות את הפלטפורמה לאחר יצירה
                </p>
              </div>
