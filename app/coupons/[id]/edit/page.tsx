  ...
  const fetchCoupon = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // טעינת וריאציות אם יש מוצר מתנה
        if (coupon.giftProductId && selectedShop) {
          try {
            const productResponse = await fetch(`/api/products/${coupon.giftProductId}`)
            if (productResponse.ok) {
              const productData = await productResponse.json()
              setGiftProductVariants(productData.variants || [])
              // שמירת המוצר ב-state כדי שיוצג גם כשלא יש חיפוש פעיל
              const giftProduct = {
                id: productData.id,
              ...
              setGiftProductSearchResults([giftProduct])
            }
          } catch (error) {
        ...
        // טעינת מוצר נדרש אם יש
        if (coupon.giftConditionProductId && selectedShop) {
          try {
            const productResponse = await fetch(`/api/products/${coupon.giftConditionProductId}`)
            if (productResponse.ok) {
              const productData = await productResponse.json()
              // שמירת המוצר ב-state כדי שיוצג גם כשלא יש חיפוש פעיל
              ...
              setRequiredProductSearchResults([requiredProduct])
            }
          } catch (error) {
            console.error("Error fetching required product:", error)
          }
        }
        ...
        setFormData({
          ...
          getDiscount: coupon.getDiscount?.toString() || "",
          payQuantity: coupon.payQuantity?.toString() || "",
          payAmount: coupon.payAmount?.toString() || "",
          nthItem: coupon.nthItem?.toString() || "",
          volumeRules: (coupon.volumeRules as Array<{ quantity: number; discount: number }>) || [],
          ...
          giftVariantId: coupon.giftVariantId || "",
          giftCondition: coupon.giftCondition || "MIN_ORDER_AMOUNT",
          giftConditionProductId: coupon.giftConditionProductId || "",
          giftConditionAmount: coupon.giftConditionAmount?.toString() || "",
          // שדות להפעלת הנחת לקוח רשום
          enableCustomerDiscount: coupon.enableCustomerDiscount ?? false,
          customerDiscountPercent: coupon.customerDiscountPercent?.toString() || "",
        })
      } else {
        toast({
          title: "שגיאה",
